...
public class GoodsControllerTest {
    ...
    void getGoods() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/api/goods/1"))
                ...
                .andExpect(jsonPath("$.product_detail_dto.product_detail_id").value(1L))
                .andExpect(jsonPath("$.product_detail_dto.detail_img_url").value("//img-cf.kurly.com/shop/data/goodsview/20180628/gv40000026292_1.jpg"))
                .andExpect(jsonPath("$.product_detail_dto.detail_title").value("\\n포슬포슬하고 고소한 맛\\n감자 1kg\\n"))
                .andExpect(jsonPath("$.product_detail_dto.detail_context").value("간단히 쪄 먹기도 좋고, 다양한 요리와 함께 곁들여 먹기도 좋은 감자는 우리 식탁에 빼놓을 수 없는 식재료지요. 탄수화물은 물론이고 단백질, 비타민C까지 풍부해 마치 곡류와 채소를 동시에 먹은 것과 같은 효과를 줍니다. 컬리는 그때그때 유명산지 감자를 가락시장에서 수급하여 보내드립니다. 포슬포슬한 식감에 고소하고 은은한 단맛이 나 볶음, 구이, 튀김 등 다양하게 요리해서 먹을 수 있어요. 매일 식탁에 올려도 질리지 않는 감자를 컬리에서 간편하게 만나보세요."))
                .andExpect(jsonPath("$.product_detail_dto.product_img_url").value("//img-cf.kurly.com/shop/data/goodsview/20210202/gv10000156055_1.jpg"));
    }
